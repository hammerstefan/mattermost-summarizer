...
- [ ] 4.1 Implement URL classification logic in orchestrator (Mattermost vs Launchpad vs GitHub vs file)
- [ ] 4.2 Implement LLM-driven reference scanning (orchestrator reads fetched text, identifies URLs)
- [ ] 4.3 Implement depth tracking (current_depth, max_reference_depth)
- [ ] 4.4 Implement cycle prevention (track followed URLs per summary operation)
- [ ] 4.5 Implement delegation routing (URL type → appropriate sub-agent)
...
- [ ] 5.1 Create `src/mattermost_summarizer/critic.py`
- [ ] 5.2 Implement `SummarizationCritic(CriticBase)` class
- [ ] 5.3 Implement `_extract_gathered_context(events)` to read delegation results
- [ ] 5.4 Implement `_extract_finish_action(events)` to read produced summary
- [ ] 5.5 Implement `_build_rubric(level)` for level-specific evaluation prompts
- [ ] 5.6 Implement `_call_critic_llm(context, summary, rubric)` for LLM evaluation
- [ ] 5.7 Configure `iterative_refinement` with default threshold 0.7, max 2 iterations
- [ ] 5.8 Wire critic into orchestrator agent
- [ ] 5.9 Test critic evaluates summary and returns score
- [ ] 5.10 Test iterative refinement loop (revision when below threshold)

## 6. Configuration

- [ ] 6.1 Add `[summarizer]` TOML section to config.py
- [ ] 6.2 Add fields: max_reference_depth (default 3), critic_enabled (default true), critic_threshold (default 0.7), critic_max_iterations (default 2)
- [ ] 6.3 Add environment variable support (MM_SUMMARIZER_MAX_REFERENCE_DEPTH, etc.)
- [ ] 6.4 Update `MattermostSummarizerConfig.from_config()` to parse [summarizer] section
- [ ] 6.5 Pass config values to orchestrator and critic
- [ ] 6.6 Test TOML config with all new fields
- [ ] 6.7 Test env var override for all new fields

## 7. Integration and Cleanup

- [ ] 7.1 Update tools/__init__.py to reflect new tool distribution (sub-agents get tools, orchestrator gets DelegateTool)
- [ ] 7.2 Update summarize.py CLI if needed for new config options
- [ ] 7.3 Run ruff check and fix any linting issues
- [ ] 7.4 Run mypy and pyright, fix any type errors
- [ ] 7.5 Write integration tests for multi-agent flow
- [ ] 7.6 Write tests for recursive reference following
- [ ] 7.7 Write tests for critic evaluation
- [ ] 7.8 Run full test suite
- [ ] 7.9 Update existing tests that relied on single-agent architecture
- [ ] 7.10 Verify agent-trace.log is readable with delegation output